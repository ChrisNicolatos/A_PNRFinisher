...
INTO #TempPNR
FROM [TravelForceCosmos].[dbo].[AirSegments] WITH (NOLOCK)
LEFT JOIN AirTicketTransactions WITH (NOLOCK)
ON AirSegments.AirTicketTransactionID = AirTicketTransactions.Id
LEFT JOIN AirTickets WITH (NOLOCK)
ON AirTickets.Id = AirTicketTransactions.AirTicketId
WHERE IssueDate >= @FromDate 
AND (@From = '...' OR (SELECT TOP 1 FromAirportID FROM AirSegments ASTo WITH (NOLOCK)
...
AND (@Stopover = '' OR (SELECT COUNT(*) FROM AirSegments ASTo
	LEFT JOIN AirTicketTransactions ATTTo WITH (NOLOCK)
	ON ATTTo.Id = ASTo.AirTicketTransactionID
	LEFT JOIN AirTickets ATTo WITH (NOLOCK)
	ON ATTo.Id = ATTTo.AirTicketID   
	LEFT JOIN #TempSTOPOVER WITH (NOLOCK)
	ON #TempSTOPOVER.Id = AirSegments.FromAirportId
	WHERE ATTo.PNRId = AirTickets.PNRId AND #TempSTOPOVER.Id IS NOT NULL) >0)
  
AND (@To = '...' OR (SELECT TOP 1 ToAirportID FROM AirSegments ASTo WITH (NOLOCK)
	LEFT JOIN AirTicketTransactions ATTTo WITH (NOLOCK)
	ON ATTTo.Id = ASTo.AirTicketTransactionID
	LEFT JOIN AirTickets ATTo WITH (NOLOCK)
	ON ATTo.Id = ATTTo.AirTicketID   
...
SELECT LTRIM(RTRIM(PNR.Code))AS Code
  ...
  LEFT JOIN AirTickets WITH (NOLOCK)
  ON AirTickets.Id = AirTicketTransactions.AirTicketId
  LEFT JOIN PNR WITH (NOLOCK)
  ON PNR.ID = PNRID
  LEFT JOIN CommercialTransactions WITH (NOLOCK)
  ON CommercialTransactions.Id = AirTicketTransactions.CommercialTransactionId
  LEFT JOIN Passengers WITH (NOLOCK)
  ON Passengers.CommercialTransactionId = AirTicketTransactions.CommercialTransactionId
  LEFT OUTER JOIN Airlines TicketAirline WITH (NOLOCK)
  ON TicketingAirlineId = TicketAirline.Id
  LEFT OUTER JOIN Airlines CarrierAirline WITH (NOLOCK)
  ...
  INTO #Temp3
  FROM #Temp2
  LEFT OUTER JOIN AirTicketTransactions  WITH (NOLOCK)
  ON AirTicketTransactions.Id = #Temp2.AirTicketTransactionId 
  LEFT JOIN CommercialTransactionValues WITH (NOLOCK)
  LEFT JOIN CommercialTransactions WITH (NOLOCK)
  ...
  LEFT JOIN CommercialTransactionValues CCSell WITH (NOLOCK)
  LEFT JOIN TFEntities  WITH (NOLOCK) ON TFEntities.Id = CommercialEntityID
